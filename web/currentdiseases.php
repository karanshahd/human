...
<body>
  ...
  $q = "select * from currentdisease where username='$uname';";
  $r = @mysqli_query ($dbc, $q);
  ?>

<style>
  .imageh{max-width:50%; max-height: 50%; margin:auto; display:block}
</style>

    <?php while($row = mysqli_fetch_array($r))
        {
            $diseasename=$row['diseasename'];
            $q1 = "select * from diseaseimages where username='$uname' and diseasename='$diseasename' and type='current';";
          ...
          echo '<span class="w3-xlarge" style="font-weight:bold">Undergoing treatment from : </span><span class="w3-xlarge" >'.$row['datefrom'].'</span><br>';
          echo '</div>';
